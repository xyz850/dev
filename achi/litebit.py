...
# ==================== CLEAN ANIMATIONS (NO SPAM) ====================
class Anim:
    @staticmethod
    def spinner(text, duration=1.5):
        # Mengganti baris berderet dengan cetak statis bersih
...
class LiteBitsTeleBot:
    ...
    def render_dashboard(self):
        ...
        out = []
        out.append(f"{Col.NEON_C}┌─ {Col.NEON_Y}ACCOUNT{Col.NEON_C} " + "─" * 37 + f"┐{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}User{Col.R}      : {Col.NEON_C}{name:<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}Balance{Col.R}   : {Col.NEON_Y}{bal_str:<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}Earned{Col.R}    : {Col.NEON_G}{earned_str:<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}Cycles{Col.R}    : {Col.WHT}{str(self.cycles):<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}Success{Col.R}   : {Col.NEON_G if rate >= 90 else Col.NEON_Y}{f'{rate:.1f}%':<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}Uptime{Col.R}    : {Col.NEON_C}{f'{eh:02d}:{em:02d}:{es:02d}':<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}│{Col.R} {Col.NEON_V}Remaining{Col.R} : {Col.NEON_O}{f'{rh:02d}:{rm:02d}:{rs:02d}':<34}{Col.NEON_C}│{Col.R}")
        out.append(f"{Col.NEON_C}└" + "─" * 46 + f"┘{Col.R}")
        return "\n".join(out)
    ...
    def setup_interactive(self):
        ...
        if valid_auth:
            print(f"{Col.NEON_C}┌─ {Col.NEON_Y}SELECT MODE{Col.NEON_C} " + "─" * 34 + f"┐{Col.R}")
